...
parser.add_argument("--depth_Z", type=float, default=2.8) # Z is the press direction
parser.add_argument("--indenter_velocity", type=float, default=50)
...
def img_generation(p_xpos_list, p_ypos_list, p_zpos_list, initial_npy, image_name):
  ...
  #npy_path = "./Results/CAT_PAWS/Real_time/depth/"+ image_name +".npy"
  ...
  #image_path = "./Results/CAT_PAWS/Real_time/sim/"+ image_name +".png"
...
parser.add_argument("--translation_z", type=float, default=5.6) # translation of the indenter model in Z
...
## Create path
if not os.path.exists("./Results/CAT_PAWS/Real_time/Deformation"):
    os.makedirs("./Results/CAT_PAWS/Real_time/Deformation") 
if not os.path.exists("Results/CAT_PAWS/Real_time/sim"):
    os.makedirs("Results/CAT_PAWS/Real_time/sim")
if not os.path.exists("Results/CAT_PAWS/Real_time/depth"):
    os.makedirs("Results/CAT_PAWS/Real_time/depth")
if not os.path.exists("Results/CAT_PAWS/Real_time/PointCloud"):
    os.makedirs("Results/CAT_PAWS/Real_time/PointCloud")
if not os.path.exists("Results/CAT_PAWS/Real_time/Force_data"):
    os.makedirs("Results/CAT_PAWS/Real_time/Force_data")
if not os.path.exists("Results/CAT_PAWS/Real_time/Visualization_Frames"):
    os.makedirs("Results/CAT_PAWS/Real_time/Visualization_Frames")

## Load Model Mesh Information
# load membrane particles 
npy_file_path = "./Abaqus_model/MESH04/CAT_PAWS.npy"
...
while window.running:
  while True:     
    ...
    camera.position(-30, 30, 100)
    camera.lookat(20, 20, 19)
    camera.up(0, 0, 1)
    ...
    canvas.scene(scene)
    # if t_ti[None]%(args.displacement_interval / (args.indenter_velocity * dt)) == 0: 
    #     window.save_image('./Results/CAT_PAWS/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png')
    #     # Open the image that was saved
    #     image_path = './Results/CAT_PAWS/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png'
