...
# this code is for marker based sensor;
# here we extract the extra marker index;
...
def findNodes(filepath):
    ...
    part_start = "*Part, name=Digitac"  # modified
    part_end = "*Solid Section, elset=_PickedSet2, material=elastomer-new"  # modified
    part_content = contents[contents.find(part_start):contents.find(part_end)]
    
    node_start = "*Node"
    node_end = "*Element, type=C3D4"
...
def find_element_nodes(filepath):
    ...
    part_start = "*Element, type=C3D41" # modified
    part_end = "*Solid Section, elset=_PickedSet2, material=elastomer-new" # modified
    part_content = contents[contents.find(part_start):contents.find(part_end)]  
    # Extract node data from the part using contents
    element_start = '*Element, type=C3D41' # modified
    element_end = '*Nset, nset=_PickedSet2, internal, generate'  
...
def find_mesh_element(filepath):
    with open(filepath, 'r') as file:
        contents = file.read()
    # Extract node data from the part using contents
    node_start = '*Elset, elset="Contact Surface", instance=Digitac-1' # modified
    node_end = '*Nset, nset="Fixed Surface", instance=Digitac-1' # modified
...
def main():
    inp_file_path = 'MESH04/DIGITAC.inp'  # modified
    npy_file_path = 'MESH04/DIGITAC.npy'  # modified
    contact_txt_path = 'MESH04/Contact_Surface_index.txt'
    boundary_txt_path = 'MESH04/Boundary_Condition_index.txt'
    marker_txt_path = 'MESH04/Marker_index.txt'
    ...
    marker_indices = extract_indices(inp_content, '*Nset, nset=Marker, instance=Digitac-1', '*Elset, elset=Marker, instance=Digitac-1')
    save_indices(marker_txt_path, marker_indices)
