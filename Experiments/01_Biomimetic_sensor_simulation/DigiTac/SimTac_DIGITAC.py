...
parser.add_argument("--object", default="Sphere") 
...
parser.add_argument("--depth_Z", type=float, default=1.8) 
...
def img_generation(p_xpos_list, p_ypos_list, p_zpos_list, initial_npy, image_name):
    ...
    pixel_u = np.round(pixel_u).astype(int)
    pixel_v = np.round(pixel_v).astype(int)
    ...
    image_path = "./Results/DIGITAC/Real_time/sim/"+ image_name +".png"
    #cv2.imwrite(image_path, img)
    cv2.imshow("tactile_image", img)
    cv2.waitKey(1)
...
parser.add_argument("--translation_z", type=float, default=23) # translation of the indenter model in Z
...
## Create path
if not os.path.exists("./Results/DIGITAC/Real_time/Deformation"):
    os.makedirs("./Results/DIGITAC/Real_time/Deformation") 
if not os.path.exists("Results/DIGITAC/Real_time/sim"):
    os.makedirs("Results/DIGITAC/Real_time/sim")
if not os.path.exists("Results/DIGITAC/Real_time/depth"):
    os.makedirs("Results/DIGITAC/Real_time/depth")
if not os.path.exists("Results/DIGITAC/Real_time/PointCloud"):
    os.makedirs("Results/DIGITAC/Real_time/PointCloud")
if not os.path.exists("Results/DIGITAC/Real_time/Force_data"):
    os.makedirs("Results/DIGITAC/Real_time/Force_data")
if not os.path.exists("Results/DIGITAC/Real_time/Visualization_Frames"):
    os.makedirs("Results/DIGITAC/Real_time/Visualization_Frames")

## Load Model Mesh Information
# load membrane particles 
npy_file_path = "./Abaqus_model/MESH04/DIGITAC.npy"
...
Marker_set_path = "./Abaqus_model/MESH04/Marker_index.txt"
Marker_list = []
with open(Marker_set_path, "r") as file:
    for line in file:
        data_3 = int(line.strip())  
        Marker_list.append(int(data_3))
Marker_set_num = len(Marker_list)
Marker_set = ti.field(dtype=int, shape=Marker_set_num)
def fill_Marker_field():
    for i in ti.static(range(Marker_set_num)):
        Marker_set[i] = Marker_list[i]-1
fill_Marker_field()
...
width = 640
height = 640
camera_position = np.array([20, 17, -3])  
...
window = ti.ui.Window("Taichi Simulation on GGUI", (1280, 960), vsync=True)
...
while window.running:
  while True:     
    ...
    canvas.scene(scene)
    # if t_ti[None]%(args.displacement_interval / (args.indenter_velocity * dt)) == 0: 
    #     window.save_image('./Results/DIGITAC/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png')
    #     # Open the image that was saved
    #     image_path = './Results/DIGITAC/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png'
