...
# choice = 'Cylinder_texture', 'Cylinder', 'CUbe_1', 'Cube_2'
parser.add_argument("--object", default="Cylinder_texture") 
...
def img_generation(p_xpos_list, p_ypos_list, p_zpos_list, initial_npy, image_name):
  ...
  i_depth = cv2.bilateralFilter(i_depth, d=6, sigmaColor=10, sigmaSpace=10)
  # save the depth map
  # npy_path = "./Results/Elephant_Truck/Real_time/depth/"+ image_name +".npy"
  ...
  image_path = "./Results/Elephant_Truck/Real_time/sim/"+ image_name +".png"
  #cv2.imwrite(image_path,img)
  cv2.imshow("tactile_image_MuJoCo",img)
  cv2.waitKey(1) 

# Fixed Parameters
parser.add_argument("--depth_Z", type=float, default=0.5) 
parser.add_argument("--translation_x", type=float, default=0) 
parser.add_argument("--translation_y", type=float, default=0) 
parser.add_argument("--translation_z", type=float, default=0) 
parser.add_argument("--particle", default="100", choices=["1","10","100"]) 
parser.add_argument("--offset_indenter_x", type=float, default=20.5)
parser.add_argument("--offset_indenter_y", type=float, default=20.0) 
parser.add_argument("--offset_indenter_z", type=float, default=29.4) 
...
parser.add_argument("--actuator_displacement", type=float, default=0.4)
...
# mujoco 
xml_file = "Connect_to_Mujoco/Elephant.xml"
with open(xml_file, 'r', encoding='utf-8') as file:
      xml_content = file.read()
pattern_object = r'(?P<start><geom type="mesh" mesh=")(?P<mesh_value>.*?)(?P<end>" name="object_geom" rgba="1 1 1 1"/>)'
...
new_pos = f'{0.001*(args.offset_indenter_x)} {0.001*args.offset_indenter_y} {0.001*args.offset_indenter_z}'
...
## Create path
if not os.path.exists("Results/Elephant_Truck/Real_time/sim"):
    os.makedirs("Results/Elephant_Truck/Real_time/sim")
if not os.path.exists("Results/Elephant_Truck/Real_time/Visualization_Frames"):
    os.makedirs("Results/Elephant_Truck/Real_time/Visualization_Frames")
if not os.path.exists("Results/Elephant_Truck/Real_time/Mujoco_Frames"):
    os.makedirs("Results/Elephant_Truck/Real_time/Mujoco_Frames")
if not os.path.exists("Results/Elephant_Truck/Real_time/depth"):
    os.makedirs("Results/Elephant_Truck/Real_time/depth")

## Load Model Mesh Information
# load membrane particles 
npy_file_path = "./Abaqus_model/MESH04/Elephant_Truck.npy"
...
Active_Surface_1_set_path = "./Abaqus_model/MESH04/Active_Surface_4_index.txt"
Active_Surface_2_set_path = "./Abaqus_model/MESH04/Active_Surface_3_index.txt"
Active_Surface_1_list = []
Active_Surface_2_list = []
with open(Active_Surface_1_set_path, "r") as file:
    for line in file:
        data_3 = int(line.strip())  
        Active_Surface_1_list.append(int(data_3))
Active_Surface_1_set_num = len(Active_Surface_1_list) 
Active_Surface_1_set = ti.field(dtype=int, shape=Active_Surface_1_set_num) 
def fill_Active_Surface_1_field():
    for i in ti.static(range(Active_Surface_1_set_num)):
        Active_Surface_1_set[i] = Active_Surface_1_list[i]-1
fill_Active_Surface_1_field()
with open(Active_Surface_2_set_path, "r") as file:
    for line in file:
        data_4 = int(line.strip())  
        Active_Surface_2_list.append(int(data_4))
Active_Surface_2_set_num = len(Active_Surface_2_list) 
Active_Surface_2_set = ti.field(dtype=int, shape=Active_Surface_2_set_num) 
def fill_Active_Surface_2_field():
    for i in ti.static(range(Active_Surface_2_set_num)):
        Active_Surface_2_set[i] = Active_Surface_2_list[i]-1
fill_Active_Surface_2_field()

# load triangle mesh
Mesh_face_path = "./Abaqus_model/MESH04/triangle_faces.npy"
...
# velocity of the membrane actuator
v_x_actuator = ti.field(dtype=float, shape=())
v_y_actuator = ti.field(dtype=float, shape=())
v_z_actuator = ti.field(dtype=float, shape=()) 
v_x_actuator[None] = 0
v_y_actuator[None] = 0
v_z_actuator[None] = 0
...
width = 640
height = 640
camera_position = np.array([20, 20, 11])  
camera_position_1 = np.array([20, 20,11])
...
def substep():
  ...
  for p in x: 
    ...
    for i in range(Active_Surface_1_set_num):
      if p== Active_Surface_1_set[i]:
        if v_y_actuator[None] != 0:
          new_v[1] = -v_y_actuator[None] # the velocity of membrane actuator
          #new_v = ti.Vector([0,0,0])

    for i in range(Active_Surface_2_set_num):
      if p== Active_Surface_2_set[i]:
        if v_y_actuator[None] != 0:
          new_v[1] = v_y_actuator[None] # the velocity of membrane actuator
          #new_v = ti.Vector([0,0,0])
...
while window.running:
  while True:     
    ...
    image = simulate()
    vel_x = sim.data.qvel[0]*1e3
    vel_y = sim.data.qvel[1]*1e3
    vel_z = sim.data.qvel[2]*1e3
    t_ti[None] += 1
    substep()
    ...
    if t_ti[None]%(args.displacement_interval / (args.indenter_velocity * dt)) == 0: 
      plt.imsave('./Results/Elephant_Truck/Real_time/Mujoco_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png', image)
      print("===========================================================")
    ...
    if distance[None]>=abs(end_position_depth): 
      ...
      v_z[None] = 0
      v_y_actuator[None] = args.indenter_velocity
    ...
    if distance[None] > abs(end_position_depth) + max(abs(end_position_shear_X), abs(end_position_shear_Y)) + args.actuator_displacement: 
      sys.exit()
    
    camera.position(90, 70, 25)
    camera.lookat(20, 20, 25)
    camera.up(0, 0, 1)
    scene.set_camera(camera)
    scene.point_light(pos=(90, 20, 60), color=(0.3, 0.3, 0.3))
    ...
    canvas.scene(scene)
    # if t_ti[None]%(args.displacement_interval / (args.indenter_velocity * dt)) == 0: 
    #     window.save_image('./Results/Elephant_Truck/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png')
    #     # Open the image that was saved
    #     image_path = './Results/Elephant_Truck/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png'
