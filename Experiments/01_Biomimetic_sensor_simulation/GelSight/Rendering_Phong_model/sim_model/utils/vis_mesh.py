...
## create an arrow
def arrow(p, v, color='red', scale=0.18, length=1):
    # print(p, v)
    r = align_vectors(
        np.array([0, 0, 1]),
        v,
...
## combine all the elements into one 3D scene and get them visualized 
def show_field(cloud_map=None, field=None, field_color=None, mesh=None, pts=None, arrows=None, paths=None,
    ...
    #scene.set_camera(angles=[0, math.pi/18, 0], center=np.array([0, 0, 0]))
    scene.camera.fov = (np.deg2rad(45), np.deg2rad(45))
