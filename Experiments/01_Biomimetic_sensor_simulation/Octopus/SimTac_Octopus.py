...
parser.add_argument("--depth_Y", type=float, default=2.9)
...
def wide_angle_camera_transfer(xp, yp, p_depth):
    ...
    for i in range(len(xp)):
        ...
        if relative_position[1] < -1:
            depth.append(d)
            pixel_v.append(fx * relative_position[0] / relative_position[1] + cx)
            pixel_u.append(-fy * relative_position[2] / relative_position[1] + cy)

    return np.array(pixel_u), np.array(pixel_v), np.array(depth)
...
def img_generation(p_xpos_list, p_ypos_list, p_zpos_list, initial_npy, image_name):
  ...
  i_depth[i_depth < 0] = 0.02 
  npy_path = "./Results/Octopus/Real_time/depth/"+ image_name +".npy"
  ...
  image_path = "./Results/Octopus/Real_time/sim/"+ image_name +".png"
...
parser.add_argument("--translation_x", type=float, default=13) # translation of the indenter model in X
parser.add_argument("--translation_y", type=float, default=2) # translation of the indenter model in Y
parser.add_argument("--translation_z", type=float, default=-2) # translation of the indenter model in Z
...
## Create path
if not os.path.exists("./Results/Octopus/Real_time/Deformation"):
    os.makedirs("./Results/Octopus/Real_time/Deformation") 
if not os.path.exists("Results/Octopus/Real_time/sim"):
    os.makedirs("Results/Octopus/Real_time/sim")
if not os.path.exists("Results/Octopus/Real_time/depth"):
    os.makedirs("Results/Octopus/Real_time/depth")
if not os.path.exists("Results/Octopus/Real_time/PointCloud"):
    os.makedirs("Results/Octopus/Real_time/PointCloud")
if not os.path.exists("Results/Octopus/Real_time/Force_data"):
    os.makedirs("Results/Octopus/Real_time/Force_data")
if not os.path.exists("Results/Octopus/Real_time/Visualization_Frames"):
    os.makedirs("Results/Octopus/Real_time/Visualization_Frames")

## Load Model Mesh Information
# load membrane particles 
npy_file_path = "./Abaqus_model/MESH04/Octopus.npy"
...
width = 640
height = 640
camera_position = np.array([20, 25.95, 25])  
...
while window.running:
  while True:     
    ...
    camera.position(100, -60, 25)
    camera.lookat(20, 20, 25)
    camera.up(0, 0, 1)
    scene.set_camera(camera)
    scene.point_light(pos=(50, -80, 0), color=(0.3, 0.3, 0.3))
    scene.ambient_light((0.5, 0.5, 0.5))
    scene.particles(x_indenter, radius=0.5, color=(142/255,182/255,156/255))
    scene.particles(x_membrane, radius=0.1, color=(217/255,79/255,51/255))
    canvas.scene(scene)
    # if t_ti[None]%(args.displacement_interval / (args.indenter_velocity * dt)) == 0: 
    #     window.save_image('./Results/Octopus/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png')
    #     # Open the image that was saved
    #     image_path = './Results/Octopus/Real_time/Visualization_Frames/' + str(args.object) + '_frame_' + str(t_ti[None]) + '.png'
