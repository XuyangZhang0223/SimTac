...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, data_folder):
        ...
        self.input_files = sorted([f for f in os.listdir(os.path.join(data_folder, 'Fine_tune_Data/MidLow_Youngs_Modulus/Input')) if f.endswith('.npy')])
        self.gt_files = sorted([f for f in os.listdir(os.path.join(data_folder, 'Fine_tune_Data/MidLow_Youngs_Modulus/Ground_Truth_Displacement')) if f.endswith('.npy')])
    ...
    def __getitem__(self, idx):
        input_path = os.path.join(self.data_folder, 'Fine_tune_Data/MidLow_Youngs_Modulus/Input', self.input_files[idx])
        gt_path = os.path.join(self.data_folder, 'Fine_tune_Data/MidLow_Youngs_Modulus/Ground_Truth_Displacement', self.gt_files[idx])
...
if __name__ == '__main__':
    ...
    # Load the model (ensure it's the same architecture as the one you trained)
    net = MinkUNet14(in_channels=3, out_channels=3, D=3)

    # Load the pre-trained weights
    pre_trained_model_path = 'models/minkunet_model_MAE_displacement_final.pth'
    ...
    # # Save losses to CSV
    # loss_data = pd.DataFrame({'Epoch': list(range(1, num_epochs + 1)), 'Training Loss': training_losses, 'Validation Loss': validation_losses})
    # loss_data.to_csv('fine_tuning_losses_Displacement_MidLow_E.csv', index=False)

    # Plot validation loss
    plt.figure(figsize=(10, 5))
    plt.plot(range(1, num_epochs + 1), training_losses, label='Training Loss', color='red')
    plt.plot(range(1, num_epochs + 1), validation_losses, label='Validation Loss', color='blue')
    plt.title('Training and Validation Loss Curves (Fine-tuning)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.grid(True)
    plt.legend()
    #plt.savefig('fine_tuning_validation_loss_curve_Displacement_Low_E.png')

    # Save the fine-tuned model
    torch.save(net.state_dict(), 'models_fine_tune/fine_tuned_minkunet_model_Displacement_MidLow_E.pth')
