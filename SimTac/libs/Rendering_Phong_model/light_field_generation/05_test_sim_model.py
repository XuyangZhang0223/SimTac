...
npy_path = os.path.join(__location__, './npy_data/')

light_fields = SimulationModel.load_assets(assets_path, fields_size, sim_size, field, 3)

stack = cv2.resize(cv2.cvtColor(cv2.cvtColor(cv2.imread(npy_path + '/bkg.png'), cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2BGR), sim_size)
...
model = SimulationModel(**{
    ...
    ## undeformed depth map
    'background_depth': cv2.resize(np.load(npy_path + 'initial_npy.npy').astype(np.float32), sim_size),
...
## deformed depth map
depths = np.load(npy_path + 'Pacman_a90H5d0.35.npy')
print(depths.shape)
...
cv2.imshow('tactile_rgb',tactile_rgb[0])
cv2.waitKey(-1)
